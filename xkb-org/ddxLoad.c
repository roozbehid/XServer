...
#ifdef WIN32
    if (asprintf(&buf,
		 "type x-keyboard.txt | \"%s%sxkbcomp\" -I%s/usr/share/X11/xkb -w %d %s -xkm %s "
	...
	int 			nameRtrnLen)
{
	...
	*xkbRtrn = NULL;
	...
	if ((names->keycodes == NULL) && (names->types == NULL) && (names->compat == NULL) && (names->symbols == NULL) && (names->geometry == NULL)) {
		LogMessage(X_ERROR, "XKB: No components provided for device %s\n",
			keybd->name ? keybd->name : "(unnamed keyboard)");
		return 0;
	}
	else
		if (!XkbDDXCompileKeymapByNames(xkb, names, want, need, nameRtrn, nameRtrnLen))
		{
