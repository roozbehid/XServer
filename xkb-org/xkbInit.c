...
XkbGetRulesDflts(XkbRMLVOSet *rmlvo)
{
    ...
    rmlvo->rules = _strdup(rmlvo->rules);
    rmlvo->model = _strdup(rmlvo->model);
    rmlvo->layout = _strdup(rmlvo->layout);
    rmlvo->variant = _strdup(rmlvo->variant);
    rmlvo->options = _strdup(rmlvo->options);
}
