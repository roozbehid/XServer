    ...
    char	*pattern)
{
    ...
    font_data->xlfd_name = _strdup(pattern);
    if(font_data->xlfd_name == NULL)
	return (-1);
    ...
    FontDataRec *font_data_return)
{
	...
	*/
	while (count-- > 0) {
		...
		*/
		if (font_data_return) {
		    font_data_return->xlfd_name = _strdup(font_data->xlfd_name);
    ...
    */
	...
	if(font_set->font_data_count > 0) {
	    ...
	    */
		 ...
		 * font_data_return.
		 */
		font_set->font_name = _strdup(font_data_return.xlfd_name);
	    ...
	    */
		...
		font_set->font_name = _strdup(font_set->substitute[i].xlfd_name);
		if(font_set->font_name == (char *) NULL)
		    goto err;
    ...
    base_name = _strdup(oc->core.base_name_list);
    if (base_name == NULL)
	goto err;
    ...
    _Xconst char *res_class)
{
    ...
    om->core.rdb = rdb;
    if (res_name) {
	om->core.res_name = _strdup(res_name);
	if (om->core.res_name == NULL)
	    goto err;
    }
    if (res_class) {
	om->core.res_class = _strdup(res_class);
